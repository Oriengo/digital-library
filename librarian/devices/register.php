...
$isTablet = strpos($userAgent, 'tablet');

if (!$isMobile && !$isTablet) {
    // redirect to a different page if the device is not a smartphone
	echo "<script>alert('The Page cant be openned with Large screen.');window.location.href='../../login/login.php';</script>";
				...
				$query = "SELECT * FROM space_bookig INNER JOIN student_information ON space_bookig.reg_number = student_information.reg_number INNER JOIN programmes ON student_information.programme_code = programmes.programme_code INNER JOIN schools ON schools.school_code = student_information.school_code INNER JOIN campuses ON student_information.campus_id = campuses.campus_id INNER JOIN classes ON student_information.class_name = classes.class_name WHERE student_information.reg_number = '$reg_number' AND booked_date = '$today'LIMIT 1 ";
						...
						<img  height='240px' width='210px' src=<?php echo "../../images/$profile_image"  ?>>
 							...
 							<div class="text-center">
 								<input type="submit" name="sub" type="submit" class="btn btn-primary" value="Allow inside">
 							</div>
 						</div>
 					</div>
 						<?php } ?>
...
<?php 
 if(isset($_POST['sub'])){
	...
	$query1 = "INSERT INTO attendance_register(booking_id, reg_number, time_in)VALUES('$booking_id','$reg_no','$current_date') ";
	$result = mysqli_query($con, $query1);
	if($result){

	   echo "<script>alert('Accepted in succesfully!');window.location.href='register.php';</script>";
		// header('location:register.php ');
	}
	$query2 = "UPDATE `space_bookig` SET att_comment = 'Attended' WHERE booking_id = '$booking_id'  ";
